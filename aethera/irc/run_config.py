...
class ProviderConfig:
    ...
    def from_dict(cls, data: dict) -> "ProviderConfig":
        ...
        return cls(
            ...
            model=data.get("model", "meta-llama/Llama-3.1-405B"),
            params=params,
...
class GenerationRunConfig:
    ...
    # Provider configs
    # Default: base model (completion) for generation via Featherless, instruct
    # model for judging via OpenRouter. See aethera.irc.config for env overrides.
    ...
    # Default False: generation defaults to a base/completion model, which must
    # not receive an instruct system prompt. Set True for instruct generation.
    ...
    def from_dict(cls, data: dict) -> "GenerationRunConfig":
        ...
        return cls(
            ...
            use_instruct_mode=data.get("use_instruct_mode", False),
...
def get_available_providers() -> list[ProviderInfo]:
    ...
    providers = [
        ProviderInfo(
            name="featherless",
            display_name="Featherless (HF router) — base models",
            models=[
                # Base/completion models — the intended generation path.
                "meta-llama/Llama-3.1-405B",
                "meta-llama/Llama-3.1-70B",
                "mistralai/Mistral-Large-Instruct-2407",
            ],
            supports_n=False,  # Use array prompts for batching if needed
            supports_caching=False,  # Backend prefix caching, not client-controlled
            requires_api_key=True,
            api_key_env_var="HF_TOKEN",
            has_api_key=bool(
                os.environ.get("HF_TOKEN")
                or os.environ.get("FEATHERLESS_API_KEY")
                or os.environ.get("IRC_FEATHERLESS_API_KEY")
            ),
        ),
        ProviderInfo(
            name="openrouter",
            display_name="OpenRouter — judge models",
            models=[
