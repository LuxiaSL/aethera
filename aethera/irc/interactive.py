...
class InteractiveGenerator:
    ...
    def _create_provider(self, provider_config: ProviderConfig) -> InferenceProvider:
        ...
        if provider_config.provider == "anthropic":
        ...
        elif provider_config.provider == "featherless":
            from .providers.openai_compatible import create_featherless_provider
            api_key = provider_config.api_key or irc_config.featherless_api_key
            if not api_key:
                raise ValueError(
                    "Featherless key not provided and HF_TOKEN / FEATHERLESS_API_KEY not set"
                )
            return create_featherless_provider(
                api_key=api_key,
                model=provider_config.model,
                base_url=provider_config.base_url or irc_config.featherless_base_url,
                # Base/completion is the intended generation path for Featherless.
            ...
            return OpenAICompatibleProvider(
                ...
                # Local providers serve base models for generation by default.
                mode=CompletionMode.COMPLETION,
                provider_name="local",
            )
        else:
