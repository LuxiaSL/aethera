...
class BatchCompletionResult:
    ...
    cost_usd: Optional[float] = None
    cached_tokens: int = 0
    # Per-completion stop reason ("length" = hit max_tokens, so the last line is
    # likely truncated mid-text; "stop"/"end_turn" = finished cleanly). Parallel
    # to texts; empty if the provider didn't report it.
...
class InferenceProvider(ABC):
    ...
    async def complete_chat(
        ...
        """
        Complete from a multi-turn message history (list of {role, content}).

        Used by the STATEFUL judge, which carries its own prior reasoning
        forward across rounds as conversation turns rather than re-deriving
        intent from the transcript each round.

        Default implementation flattens the history into a single prompt and
        delegates to complete(), so any provider works out of the box.
        Providers with a native chat endpoint (e.g. OpenRouter) override this to
        preserve the role structure and cross-turn prompt caching that make the
        append-only judge conversation cheap.

        Args:
            messages: Ordered chat turns, each {"role": ..., "content": ...}.
        ...
        Returns:
            CompletionResult with the latest assistant message text
        """
    ...
    def _flatten_messages(messages: list[dict]) -> str:
        """
        Flatten a chat history into a single prompt string (fallback path).

        Content blocks (e.g. cache_control-wrapped text) are reduced to their
        text. The trailing "ASSISTANT:" cues a base/completion model to answer.
        """
        parts: list[str] = []
        for m in messages:
