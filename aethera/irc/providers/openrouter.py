...
class OpenRouterProvider(InferenceProvider):
    ...
    def __init__(
        ...
        Args:
            ...
            provider_routing: Optional OpenRouter `provider` routing object, e.g.
                {"sort": "throughput"} to route to the fastest backend, or
                {"order": ["moonshotai"], "allow_fallbacks": False} to pin one.
                Without this, OpenRouter load-balances across backends of wildly
                varying throughput (and prompt caching can't stick).
        """
    ...
    async def complete_chat(
        ...
        """
        Complete from a full multi-turn message history via /chat/completions.

        The stateful judge depends on this: the message list is passed straight
        through so the judge's prior turns (its carried-forward intent) stay in
        context. Prompt caching is automatic for Moonshot/DeepSeek/OpenAI
        backends, so the growing-but-append-only history is mostly cache hits
        and stays cheap — provided the judge is pinned to one backend (see
        provider_routing). Falls back to the flattened single-prompt path for
        COMPLETION-mode models.
        """
        ...
        # Reasoning-capable judges may return content=null with output in a
        # `reasoning` field — coalesce so we never get None.
        ...
        if cached_tokens > 0:
            logger.debug(
                f"OpenRouter stateful chat cache hit: {cached_tokens} tokens"
            )
        ...
        return CompletionResult(
            ...
            cost_usd=self._cost_from_usage(usage),
    ...
    async def _do_completion(
        ...
        if self._mode == CompletionMode.COMPLETION:
            ...
            # Some models (esp. reasoning-capable ones) return content=null and
            # put the output in a `reasoning` field. Coalesce so we never get None.
            _msg = data["choices"][0]["message"]
            text = _msg.get("content") or _msg.get("reasoning") or ""
            usage = data.get("usage", {})
    ...
    def _cost_from_usage(self, usage: dict) -> float:
        """Prefer OpenRouter's REAL charged cost (returned in usage.cost when
        usage.include=true, which we request), else fall back to the rate table."""
        real = usage.get("cost")
        if real is not None:
            try:
                return float(real)
    ...
    def _estimate_cost(self, usage: dict) -> float:
        ...
        COSTS_PER_1K = {
            ...
            # Moonshot Kimi (the IRC judge; modelrun/fp4 backend = $0.40/$1.90 per M)
            "moonshotai/kimi-k2.5": (0.0004, 0.0019),
            "moonshotai/kimi-k2": (0.0004, 0.0019),
        }
